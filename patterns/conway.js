 ...
 */
  ...
  var GOL = {
    ...
    // Initial state
    initialState : '[{"28":[20]},{"29":[20]},{"30":[20]},{"31":[19]},{"33":[18,20]},{"34":[20]}]',
    ...
    grid : {
      ...
      {
        color : '#F3F3F3' // Special case: 0px grid
      }
      ]
    },
    ...
    colors : {
      ...
      {
        dead : '#000000',
       // trail: ['#FF0000', '#DD0000', '#BB0000', '#990000', '#770000', '#550000', '#330000', '#110000'],
//          alive: ['#FF0000', '#4B0082', '#8F00FF',  '#0000FF', '#FFFF00', '#00FF00', '#FF7F00'],
//        trail : ['#9898FF', '#8585FF', '#7272FF', '#5F5FFF', '#4C4CFF', '#3939FF', '#2626FF', '#1313FF', '#0000FF'],
//                 '#FF1313', '#FF2626', '#FF3939', '#FF4C4C', '#FF5F5F', '#FF7272', '#FF8585'],
            //, '#1313FF', '#2626FF', '#3939FF', '#4C4CFF', '#5F5FFF', '#7272FF', '#8585FF'],
	alive: ['#0000FF'],
        trail : ['#000005'] //, '#858500', '#727200', '#5F5F00', '#4C4C00', '#393900', '#262600', '#131300', '#000000', '#131300', '#262600', '#393900', '#4C4C00', '#5F5F00', '#727200', '#FF0000']
      },
      {
        dead : '#FFFFFF',
        trail : ['#B5ECA2'],
    ...
    canvas: {
      ...
      drawCell : function (i, j, alive) {
        ...
        if (alive) {
          ...
          if (this.age[i][j] > -1)
            fillStyle = GOL.colors.schemes[GOL.colors.current].alive[this.age[i][j] % GOL.colors.schemes[GOL.colors.current].alive.length];

        } else {
          if (GOL.trail.current && this.age[i][j] < 0) {
            fillStyle = GOL.colors.schemes[GOL.colors.current].trail[(this.age[i][j] * -1) % GOL.colors.schemes[GOL.colors.current].trail.length];
          } else {
            fillStyle = GOL.colors.schemes[GOL.colors.current].dead;
          }
        }

        //this.context.fillRect(this.cellSpace + (this.cellSpace * i) + (this.cellSize * i), this.cellSpace + (this.cellSpace * j) + (this.cellSize * j), this.cellSize, this.cellSize);
        memgrid[i][j] = fillStyle;
...
var refreshrate = 10;
  ...
  setInterval(function () { GOL.handlers.buttons.run(); GOL.handlers.buttons.clear(); GOL.init(); GOL.handlers.buttons.run(); }, 20000);
  ...
  setInterval(function () { 
     ...
     var message = new Buffer(16182);
      ...
      var cou = 0;
      var out = '';
      for (var y = 0,x; y < 93;  y++) {
        for (x = 0; x < 58; x++) {
           col = memgrid[x][y];
           red = String.fromCharCode(Math.floor(parseInt(col.substr(1,2), 16) / 2) + 128);
           green = String.fromCharCode(Math.floor(parseInt(col.substr(3,2), 16) / 2) + 128);
           blue = String.fromCharCode(Math.floor(parseInt(col.substr(5,2), 16) / 2) + 128);
           out = out + col;
	   var pixel = blue + red + green;
